...
abstract class RDD[T: ClassTag](
  ...
  //  RDD checkpoint之后得到了一个新的RDD，那么child RDD
  // 如何知道 parent RDD 有没有被checkpoint过呢？ 看 RDD的源码，我们可以发现：
  ...
  final def iterator(split: Partition, context: TaskContext): Iterator[T] = {
    // 只有rdd设置了cache或persist时，storageLevel才不是NONE，可以调用getOrCompute方法，
    // 首先从BlockManager中直接获取，如果没有再通过compute方法计算获取并保存到BlockManager中
  ...
  private[spark] def getOrCompute(partition: Partition, context: TaskContext): Iterator[T] = {
    ...
    /**
      * 由于该方法有Executor调用，可使用SparkEnv代替sc.env
      * 根据数据块Block编号先读取数据，然后再更新数据，这里是读取数据的入口
      */
    SparkEnv.get.blockManager.getOrElseUpdate(blockId, storageLevel, elementClassTag, () => {
      ...
      //对getOrElseUpdate返回结果进行处理，该结果表示处理成功记录结果度量信息
      ...
      //对getOrElseUpdate返回结果进行处理，该结果表示
      //保存失败，例如数据太大无法放到内存中并且，也无
      //法保存在磁盘中，把该结果返回调用者，由其决定如
      //何处理
      case Right(iter) =>
  ...
  /**
    *
    * 参数定义：
    * 首先可以看到，有个U类型的参数叫做zeroValue，然后有两个方法参数，第一个是seqOp: (U, T) => U
    * 将U和T类型的数据转化为T类型的数据，第二个函数combOp: (U, U) => U将两个U类型的数据转化为U类型，
    * 返回的是一个U类型的数据。
    * 参数作用：
  ...
  def aggregate[U: ClassTag](zeroValue: U)(seqOp: (U, T) => U, combOp: (U, U) => U): U = withScope {
    ...
    /**
      * 这个源码就是针对每个RDD分区，进行执行的时候的函数，因为实际上每个分区最终都是一个迭代器，然后执行迭代器的aggregate，
      * 参数也是我们给定的参数。Iterator 的aggregate方法实际上三个参数是没有用到的，也即CombOp没有用到。
      */
  ...
  /**
    ...
    */
  //　程序第一步就是判断checkpointDir是否为空，如果为空直接抛出异常，
  // 而这个checkpointDir是由上面的setCheckpointDir函数设置的。这里
  // 我们应该设置了checkpointDir，所以直接判断checkpointData.isEmpty
  // 是否成立，checkpointData是什么东西呢？
  def checkpoint(): Unit = RDDCheckpointData.synchronized {
    ...
    if (context.checkpointDir.isEmpty) {
      ...
      // 调用 Checkpoint 方法， 构建 checkpointData
