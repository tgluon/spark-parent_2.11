...
class SparkContext(config: SparkConf) extends Logging {
  ...
  //  当spark程序启动时，SparkContext会创建Driver端的SparkEnv
  ...
  /**
    ...
    * hadoofile方法的调用会创建一个hadoopRDD，其中元素，其实就是(key,value) pair
    * key是hdfs或文本文件的每一行的offset，value就是文本行，然后对HadoopRDD调用map()
    * 方法,会剔除key，只保留value，然后获取一个MapPartitionsRDDMapPartitionRDD内部的
    * 元素，其实就是一行一行的文本行
    */
  def textFile(
                path: String,
  ...
  def setCheckpointDir(directory: String) {
    ...
    // 如果是非local模式，directory要求是HDFS上的目录。事实上，如果你是非local模式，
    // 但是指定的checkpint路径是本地路径，程序运行的时候会出现异常
  ...
  // NOTE: masthis must be placed at the end of the SparkContext constructor.
