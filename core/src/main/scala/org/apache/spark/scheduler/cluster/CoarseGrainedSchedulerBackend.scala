...
class CoarseGrainedSchedulerBackend(scheduler: TaskSchedulerImpl, val rpcEnv: RpcEnv)
  ...
  class DriverEndpoint(override val rpcEnv: RpcEnv, sparkProperties: Seq[(String, String)])
    ...
    override def receiveAndReply(context: RpcCallContext): PartialFunction[Any, Unit] = {
      ...
      case RegisterExecutor(executorId, executorRef, hostname, cores, logUrls) =>
        if (executorDataMap.contains(executorId)) {
          ...
          // 回复Executor完成注册消息并在监听总线中添加Executor事件
    ...
    //第一步，调用TaskSchedulerImpl的resourceOffers()方法，执行任务分配算法
    //将各个task分配到executor上去
    // 第二步，分配好task到executor之后，执行自己的launchTasks()方法，将分配的
    //task发送LaunchTask消息到对应的executor上去，有executor启动并执行task
    ...
    private def makeOffers() {
      ...
      //传入的是application所有可用的executor，并且将其封装WorkerOffer
      //代表了每个executor可用的cpu资源数量
