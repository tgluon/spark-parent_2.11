...
/**
  * 在TaskScheduler中，对一个单独的TaskSet任务进行调度。
  * 这个类负责追踪每一个task，如果task失败的话，会负责重
  * 试task，直到超过重试的次数限制，并且会通过延迟调度，为
  * 这个TaskSet处理本地化调度机制，它的主要接口是resourceOffer，
  * 在这个接口中，TaskSet会希望在一个节点上运行一个任务，并且接受
  * 任务的状态改变消息，来直到它负责的task的状态改变了
...
private[spark] class TaskSetManager(
  ...
  /**
    ...
    */
  //这个方法大致的意思，就是说，会判断这个executor在这个本地化几倍，之前
  //的等待时间是多少如果说，本地化级别的等待时间再一定范围内，那么就认为
  //task使用本地化级别可以在executor上启动
