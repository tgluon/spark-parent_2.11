...
/**
  ...
  */
//Task有两个子类，一个是非最后的Stage的Task，ShuffleMapTask；
// 一个是最后的Stage的Task，ResultTask。它们都覆盖了Task的runTask方法。
private[spark] abstract class Task[T](
  ...
  final def run(
    ...
    /**
      ...
      * 比如说，task重试几次，包括，task属于哪个stage，
      * task要处理的是rdd的那个partition，等等。
      */
  ...
  /**
    * 调用到了抽象方法，那就意味着什么，这个类，只是一个模板，或者抽象类
    * 父类，仅仅封装了一些子类通用的数据和操作，而关键的操作，全部都要依
    * 赖于子类的实现，
    * Task的子类，有哪些实现？
    * ShuffleMapTask,TesultTask,要运行他们的runTask()方法，才能执行我们定义
    * 的算子和逻辑
    *
    * @param context
    * @return
