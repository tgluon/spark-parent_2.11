...
object SparkEnv extends Logging {
  ...
  private def create(
    ...
    val blockTransferService = new NettyBlockTransferService(conf, securityManager,
      bindAddress, advertiseAddress, blockManagerPort, numUsableCores)
    //创建BlockManagerMaster，如果Driver端在BlockManagerMaster内部，则创建
    //终端点BlockManagerMasterEndPoint；如果是Excutor，则创建BlockManagerMasteEndPoint的引用
    val blockManagerMaster = new BlockManagerMaster(registerOrLookupEndpoint(
      BlockManagerMaster.DRIVER_ENDPOINT_NAME,
      new BlockManagerMasterEndpoint(rpcEnv, isLocal, conf, listenerBus)),
      conf, isDriver)
    //创建BlockManager，如果是Driver端包含BlockManagerMaster，如果是Executor
    //包含的是BlockManagerMaster的引用，另外BlockManager包含了远程数据传输服务，
    //当BlockManager调用initialize()方法初始化时真正生效
