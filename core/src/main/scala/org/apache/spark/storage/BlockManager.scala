...
/**
  ...
  * BlockManager运行在每一个节点上，包括driver和executor，都会有一份，主要提供了
  * 了本地或者远程数据的功能，支持内存，磁盘，堆外存储
  */
private[spark] class BlockManager(
  ...
  def initialize(appId: String): Unit = {
    ...
    //初始化，用于进行远程block数据传输的BlockTransferService
    //在Exexutor中启动远程数据传输服务，根据配置启动传输服务器BlockTransferServer,在该服务器启动后等待其他节点发送请求服务
    ...
    //为当前这个BlockManager，创建一个唯一的BlockManagerId使用到了那些东西？
    //executor(每个BlockManager都关联一哥Executor，BlockTransferService的hostname
    //以及BlockTransferService的port)，所以，从BlockManagerId的初始化即可看出，一个
    //BlockManager是通过一个节点上的Executor来唯一标识的。
    // 构建blockManagerId
    val id = BlockManagerId(executorId, blockTransferService.hostName, blockTransferService.port, None)
   //把Executor的BlockManager注册到BlockManager中，其中包括其终端点BlockMangerSlaveEndpoint的引用，
   // Master端持有该引用可以向Executor发送消息
    ...
    // 获取Shuffle编号，如果启动外部Shuffle服务，则加入外部Shuffle服务端口号信息，否则使用BlockManger编号
  ...
  /**
    ...
    * will be made then.
    */
//  reportAllBlocks 报告BlockManager所有的数据块
  ...
  /**
    ...
    */
//  reregister 注册BlockManager，并且报告BlockManager所有数据块的状态
  def reregister(): Unit = {
    //把Executor的BlockManager注册到BlockManagerMaster中
    //其中包含其终端点，BlockManagerSlavaeEndpoint的引用
    //Master端持有引用可以向Executor发送消息
...
//  getBlockData 获取本地数据块数据
    ...
    blockInfoManager.lockForReading(blockId) match {
      ...
      case Some(info) =>
        ...
        if (level.useMemory && memoryStore.contains(blockId)) {
          ...
          //从磁盘中获取数据，由于保存到磁盘的数据是序列化的，读取的到的数据也是序列化
        } else if (level.useDisk && diskStore.contains(blockId)) {
          val iterToReturn: Iterator[Any] = {
            val diskBytes = diskStore.getBytes(blockId)
            if (level.deserialized) {
              //如果存储几倍需要反序列化，则把读取的数据反序列化，然后存储到内存中
  ...
  private def getLocations(blockId: BlockId): Seq[BlockManagerId] = {
    // 向BlockManagerMasterEndpoint终端点发送消息，获取数据块所在节点信息
  ...
  def getRemoteBytes(blockId: BlockId): Option[ChunkedByteBuffer] = {
    ...
    while (locationIterator.hasNext) {
      ...
      //通过BlockRransferService提供的fetchBlockSysnc方法获取远程数据
      val data = try {
  ...
  /**
    ...
    */
//  获取集群中非当前BlockManagerId和非Driver端的BlockManagerId的所有BlockManagerId
...
//  replicate 复制数据块到其他节点
  ...
  /**
    ...
    */
//  dropFromMemory 从内存中放弃某一个block,可能内存已经满了，放在磁盘比较合适等
