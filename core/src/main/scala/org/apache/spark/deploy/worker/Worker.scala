...
//管理一个driver的执行，包括在driver失败时自动重启driver，目前这种方式仅仅时候standalone模式
private[deploy] class Worker(
    ...
    case LaunchExecutor(masterUrl, appId, execId, appDesc, cores_, memory_) =>
      if (masterUrl != activeMasterUrl) {
        ...
        try {
          ...
          //通过SPARK_EXECUTOR_DIRS环境变量，在Worker中创建
          //Executor执行目录，当程序执行完毕后有Worker进行删除
          ...
          // 在ExecutorRunner中创建CoarseGrainedExecutorBackend
          // 对象，创建的是使用应用信息中的command，而在command
          // 在SparkSchedulerBackend的start方法中构建
  ...
  private[worker] def handleDriverStateChanged(driverStateChanged: DriverStateChanged): Unit = {
    ...
    //driver执行完成后，DriverRunner线程会发送一个状态给worker
