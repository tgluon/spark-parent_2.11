...
private[deploy] class ExecutorRunner(
  ...
  private def fetchAndRunExecutor() {
    try {
      ...
      // 启动构建器，创建CoarseGrainedExecutorBackend实例
      ...
      // 等待CoarseGrainedExecutorBackend运行结束，当结束是
      // 向Worker发送退出状态信息
      ...
      // 向Executor线程所属的Worker endpoint,发送ExecutorStateChanged消息
