...
private[streaming] class BlockGenerator(
  ...
  //blockIntervalMs是有一个默认值，默认是200ms
  private val blockIntervalMs = conf.getTimeAsMs("spark.streaming.blockInterval", "200ms")
  require(blockIntervalMs > 0, s"'spark.streaming.blockInterval' should be a positive value")
  // 相当于每个200ms就会去调用updateCurrentBuffer函数
  private val blockIntervalTimer =
    new RecurringTimer(clock, blockIntervalMs, updateCurrentBuffer, "BlockGenerator")
  //队列大小
  private val blockQueueSize = conf.getInt("spark.streaming.blockQueueSize", 10)
  private val blocksForPushing = new ArrayBlockingQueue[Block](blockQueueSize)
  //后台线程启动就会调用keepPushingBlocks
  private val blockPushingThread = new Thread() {
    override def run() {
      //每隔一段时间就会到blocksForPushing去block
      keepPushingBlocks()
    }
  }
  // 用于存放原始数据
  ...
  //启动一个数据块生成定时器，将当前currentBuffer缓存中的数据按照用户
  //在spark Streaming应用程序里定义的批处理时间间隔封装成一个block数据
  //块，然后存放到BlockGenerator的blockForPushing队列中；第二是启动一个
  //blockPushingThread线程，不断地将BlockForPush队列中的数据块传递个BlockManager
  def start(): Unit = synchronized {
    if (state == Initialized) {
      state = Active
  //开启一个定时器，定期地把缓存中的数据封装成数据块
      blockIntervalTimer.start()
      //开启一个线程，不断地将封装好的数据推送给BlockManger
