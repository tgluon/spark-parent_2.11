...
private[streaming] class ReceiverSupervisorImpl(
  ...
  private val receivedBlockHandler: ReceivedBlockHandler = {
    ...
    if (WriteAheadLogUtils.enableReceiverLog(env.conf)) {
      ...
      // 如果开启预写日志spark.streaming.receiver.writeAheadLog.enable=true
      // 那么receivedBlockHandler就是WriteAheadLogBasedBlockHandler；如果没
      // 有开启预写日志，那么receivedBlockHandler就是BlockManagerBasedBlockHandler
  ...
  def pushAndReportBlock(
    ...
    //存储block到blockManager中,这里可以看到预写日志
