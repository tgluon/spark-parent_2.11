...
class JobScheduler(val ssc: StreamingContext) extends Logging {
  ...
  def start(): Unit = synchronized {
    ...
    listenerBus.start()
    // 创建ReceiverTracker 数据接收器(Receiver)的管理者
    ...
    //启动receiverTracker
    //启动DStream关联的Receiver，逻辑在ReceiverTracker的start()方法中
  ...
  def submitJobSet(jobSet: JobSet) {
    if (jobSet.jobs.isEmpty) {
      ...
      //将每个 job 都在 jobExecutor 线程池中、用 new JobHandler 来处理
  ...
  private class JobHandler(job: Job) extends Runnable with Logging {
    ...
    /**
      ...
      * 这里就与我们在 [DStream 生成 RDD 实例详解](1.2 DStream 生成
      * RDD 实例详解.md) 里分析的对应起来了： 在 ForEachDStream.generateJob(time) 时，
      * 是定义了 Job 的运行逻辑，即定义了 Job.func。而在 JobHandler 这里，是真正调用了 Job.run()、
      * 将触发 Job.func 的真正执行！
      */
    def run() {
