...
/**
  ...
  */
//在创建和完成StreamingContext的初始化之后，创建了DStreamGraph、
//JobScheduler等关键组件之后，就会调用StreamContext的socketTextStream等方法，来
//创建输入DStream，然后对输入的DStream执行一系列的transformation转换操作
//最后，会执行一个outout输出操作，来触发针对一个一个的batch的job的触发和执行

//上述初始化操作完成之后，有一个方法必须调用StreamingContext.start()
//方法，启动一个Spark Streaming应用程序，这个start()方法，其实最重要
//的，就是创建StreamingContext相关的另外两个最重要的组件：ReciverTracker
//和JobGenerator，另外，最重要的是启动整个Spark Streaming应用程序的输入
//DStream，对应Receiver，Receiver是在Woker节点上的Excutor中启动
class StreamingContext private[streaming](
  ...
  //JobGenerator会负责每个batch interval，生成一个job，然后通过Jobscheduler
  //来调度和提交job，如果自定义了reduceBykey这种算子，还是会有shuffle，而且
  //底层的数据存取组件，还是executor关联的BlockManager，负责持久化数据存储的
  //高层组件，还是CatcheManager
